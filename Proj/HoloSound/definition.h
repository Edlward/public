...
//定义各状态名称的头文件  20159812 王领      --refactor by lizulin 2017见历史版本
...
//NOTE:每帧最大采样点数,48Khz时为2000,96K时为4000,若未来需要支持96k,记得修改此值
#define PER_FRAME_SAMPLES_MAX (2000)   


#define RING_BUFFER_NUM    (8)                                           //音频环形缓冲区个数
#define RING_MEM_NUM       ((RING_BUFFER_NUM*2)+2)     //*2以指针形式使用时为了避免环形覆盖,理论上*2+1

//#define RING_MEM_NUM		((RING_BUFFER_NUM*20))        //暂时如此,待续


#ifdef OS_ARM_LINUX
#define PRE_FIX "/etc/leonis/"
#define LHC_FILE_PATH "/etc/leonis/conf.xml"                  //影厅配置文件默认保存路径
#define MACRO_FILE_PATH "/etc/leonis/macro.xml"          //待整理
#define SETTINGS_FILE_PATH "/etc/leonis/settings.ini"
#define PINKNOISE_FILE_PATH "/etc/leonis/TestWavePinkNoise10S_48Khz.wav"
#define TESTMUSE1_FILE_PATH "/etc/leonis/TestWave_Music1_48khz.wav"
#define TESTMUSE2_FILE_PATH "/etc/leonis/TestWave_Music2_48khz.wav"
#define USERINFO_FILE_PATH "/etc/leonis/UserInfo.xml"
#define SYSTEMDAT_FILE_PATH "/etc/leonis/system.dat"
#define SYSTEMCA_FILE_PATH   "/etc/leonis/CA.PEM"
#define SSL_CERT_FILE_PATH "/etc/leonis/publickey.cer"
#define SSL_KEY_FILE_PATH "/etc/leonis/privatekey.pem"
#define SRV_CHAIN_FILE_PATH "/etc/leonis/server-certificate-chain.pem"
#define SRV_KEY_FILE_PATH "/etc/leonis/server-private-key.pem"
#else
#define PRE_FIX "./"
#define LHC_FILE_PATH "./conf.xml"                  //影厅配置文件默认保存路径
#define MACRO_FILE_PATH "./macro.xml"          //待整理
#define SETTINGS_FILE_PATH "settings.ini"
#define PINKNOISE_FILE_PATH "TestWavePinkNoise10S_48Khz.wav"
#define TESTMUSE1_FILE_PATH "TestWave_Music1_48khz.wav"
#define TESTMUSE2_FILE_PATH "TestWave_Music2_48khz.wav"
#define USERINFO_FILE_PATH "UserInfo.xml"
#define SYSTEMDAT_FILE_PATH "system.dat"
#define SYSTEMCA_FILE_PATH  "CA.PEM"
#define SSL_CERT_FILE_PATH "publickey.cer"
#define SSL_KEY_FILE_PATH "privatekey.pem"
#define SRV_CHAIN_FILE_PATH "server-certificate-chain.pem"
#define SRV_KEY_FILE_PATH "server-private-key.pem"
#endif
...
{
    if (SampleRate== SRate48K||SampleRate== SRate96K)
    {
        return true;
    }
    return false;
...
//Dcap-UI登陆返回取得的权限 >=0表示成功的情况
enum ELoginPrivileges
{
	LoginPri_Other_login=-3,    //其它UI已经登录
