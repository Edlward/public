...
typedef struct _DecStatus_
{
    ...
    _DecStatus_()
    {
        AudioType=1;
        SpkNum=0;
        //memset(&AudioLevel,0,sizeof(AudioLevel)*sizeof(unsigned short int));//错误写法
        memset(AudioLevel,0,sizeof(AudioLevel));
    }
}DecStatus;

typedef struct _DecStatusPkg_{
    char Cmd[10];                              //MyStatus”
    char IDCode[20];                          //设备标识字   “DCAP-OBIA”  IdentifyingCode IDcode
    ...
    _DecStatusPkg_()
    {
        ...
        char tmpIdentifyingCode[sizeof(IDCode)]="DCAP-OBIA";
