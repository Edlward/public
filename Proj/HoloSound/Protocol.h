...
//客户端类型枚举，服务端用来区分客户端类型 Protool插件,Monitor,MPU-UI,DCAP-UI等
//不同客户端类型在登陆服务器时的初始化与断开清理操作不同
typedef struct _ClientTypeInfo_
{
    int ClientType;
}ClientTypeInfo;
...
enum EClientType
{
    ...
    //历史遗留问题,为了二进制兼容，暂时如此,待重新编译签名panner插件后废弃,令TypeBegin=0即可。
    CLIENT_TypeBegin=0x0F,
    CLIENT_UNKNOW=0x10,    //初始化时的未知状态
    CLIENT_PANNER,               //protool的panner插件
    CLIENT_MONITOR,            //Monitor对象监视UI
    CLIENT_MPUUI,                 //MPU-UIi制作端
    CLIENT_DCAPUI,                //DCAP-UI影院播放端
    CLIENT_DESIGNER,             //影院扬声器配置文件设计工具
	CLIENT_AUTOEQUI,            //AutoEQ客户端UI，理论应该同时兼容DCAP/MPU
    CLIENT_TypeEnd,
    CLIENT_TypeSIZE=CLIENT_TypeEnd-CLIENT_TypeBegin+1,
};
inline const char* ClientTypetoString(int clientType)
{
    switch (clientType)
    {
    case  CLIENT_PANNER:   return "Client_Panner";break;
    case  CLIENT_MONITOR:   return "Client_Monitor";break;
    case  CLIENT_MPUUI:   return "Client_MPU-UI";break;
    case  CLIENT_DCAPUI:   return "Client_DCAP-UI";break;
    case  CLIENT_DESIGNER:   return "Client_Designer";break;
    case  CLIENT_AUTOEQUI:   return "Client_AUTOEQ-UI";break;
    default:   return "Unknow Client";break;
    }
}
...
{
    //协议汇总
    //定义新报文的时候，可以暂不指定key的值, 只需要确认枚举名，  在对接时候同步此enum PROTOCOL结构即可 by lizulin
    //待项目稳定时候再填充协议key值到设计方案,
   ...
   CMD_VOLUME_FADER_NUM_RESP,
    ...
    CMD_GET_INPUT_CHS_VOLUME,                    //实时获取系统声卡 输入口10个bed声道的音量电平
    CMD_GET_INPUT_CHS_VOLUME_RESP,           //返回流第一个int存放扬声器个数n，后面跟着n个float,存放0-100的音量百分比
    CMD_GET_OUTPUT_CHS_VOLUME,                 //实时获取系统声卡 输出口各声道的音量电平
    CMD_GET_OUTPUT_CHS_VOLUME_RESP,	      //返回流第一个int存放扬声器个数n，后面跟着n个float,存放0-100的音量百分比

    //MPU与Designer之间的通讯
    //自动均衡相关,以下很多命令已经被废弃并出现设计，待有空清理
    ...
    CMD_SET_PM_OR_MXF,     //当前活动PM或MXF文件路径
    ...
    CMD_GET_SPREAD_SIDE_SROUND_STATE,	//控制是否扩展5.1环绕声音到9.1的顶环
    CMD_SET_SPREAD_SIDE_SROUND_STATE,
    CMD_SPREAD_SIDE_SROUND_STATE_RESP,

    CMD_GET_BASS_MANAGER_STATE,	//低音管理状态使能
    CMD_SET_BASS_MANAGER_STATE,
    CMD_BASS_MANAGER_STATE_RESP,

    CMD_GET_INPUT_CHANNEL_MAP,
    CMD_SET_INPUT_CHANNEL_MAP,
    CMD_INPUT_CHANNEL_MAP_RESP,
...
//#endif
    ...
    CMD_GET_PROGRAM_LIST_FTP,
    CMD_GET_PROGRAM_LIST_FTP_RESP,
    CMD_GET_PROGRAM_LIST_NFS,
    CMD_GET_PROGRAM_LIST_NFS_RESP,

    //通过类命令:服务端,主动发起通知,不需要客户端请求
    CMD_NOTIFY_CONTENT_OPERATION_BEGIN,    //DCP内容操作扫描/预处理等,开始进行。UI收到后禁止操作内容管理相关按钮
    CMD_NOTIFY_CONTENT_OPERATION_FINISH,   //内容操作处理完成
    ...
    CMD_GET_KDM_LIST_FTP,
    CMD_GET_KDM_LIST_FTP_RESP,
    CMD_GET_KDM_LIST_NFS,
    CMD_GET_KDM_LIST_NFS_RESP,
    CMD_COPY_KDM,                          //copy kdm file form Usb to HDD
    CMD_COPY_KDM_RESP,
    CMD_DELETE_KDM,                       //delete kdm file of HDD
    CMD_DELETE_KDM_RESP,
    CMD_COPY_PROGRAM,
	...
	CMD_MOUNT_FTP_RESP,
	CMD_UNMOUNT_FTP,
	CMD_UNMOUNT_FTP_RESP,
	CMD_UPDATE_PROGRAM_LIST_FTP,
	CMD_UPDATE_PROGRAM_LIST_FTP_RESP,
	CMD_MOUNT_NFS,
	CMD_MOUNT_NFS_RESP,
	CMD_UNMOUNT_NFS,
	CMD_UNMOUNT_NFS_RESP,
	CMD_UPDATE_PROGRAM_LIST_NFS,
	CMD_UPDATE_PROGRAM_LIST_NFS_RESP,

    //CMD_UI_BEGIN=0X9000,
    //CMD_UI_END,


    //均衡调节页面UI与Core交互命令,MPU-UI与DCAP-UI完全相同共用以下命令。
    CMD_BEGIN_EQ=0XA000,
	...
	//复位所有扬场器
	CMD_RESET_ALL_SPEAKER_EQCOEFF,
	CMD_RESET_ALL_SPEAKER_EQCOEFF_RESP,

	//批量设置均衡系数,可以多种功能复用
	CMD_BATCH_SET_EQCOEFF,
	CMD_BATCH_SET_EQCOEFF_RESP,
    ...
    CMD_GET_FADERSTATE=0XF503,          //fader音量管理器控制总音量,根据新设定，此音量值不保存到配置文件
...
#if 1
    CMD_SET_MIC1_MUTESTATE=0xF50D,
    CMD_GET_MIC1_MUTESTATE=0xF50E,
    CMD_MIC1_MUTESTATE_RESP=0XF50F,
    CMD_SET_MIC2_MUTESTATE=0xF510,
    CMD_GET_MIC2_MUTESTATE=0xF511,
    CMD_MIC2_MUTESTATE_RESP=0XF512,
#endif // 0


    CMD_GET_BOOT_VOLUME=0XF513,          //设置开机音量,保存到配置文件，下次启动用此值
    CMD_SET_BOOT_VOLUME=0XF514,
    CMD_BOOT_VOLUME_RESP=0XF515,


    CMD_FTP_INGEST = 0xF516,
    CMD_FTP_INGEST_RESP=0xF517,
    CMD_FTP_INGEST_PROGRESS=0xF518,
    CMD_FTP_INGEST_PROGRESS_RESP=0xF519,
