...
//#include "socketdef.h"	// added by liang 2008.06.06
    ...
    //new add by lizulin from androlua  可能无用,暂留 20200325
    #ifndef IPV6_V6ONLY
    ...
    #define AI_NUMERICSERV (0)
		...
		* BSD include files
        ...
        //add from androlua socket module by lizulin 20200325
...
//const char *socket_gaistrerror(int err);   //add from androlua by lizulin 20203025
...
int socket_recv(p_socket ps, char *data, size_t count, size_t *got, p_timeout tm);
 ...
 //add from androlua by lizulin 20203025--原实现仅有linux版，windows调用send/recv实现
...
//TODO:by lizulin
bool setsocketopt_not_delay(bool enable);
...
public:
    ...
    bool setSockoptNotDelay(bool enable){return setsocketopt_not_delay(enable);};
...
//#endif
    ...
    //通用读写数据接口再次封装--add by lizulin 20200325
    //返回值大于零返回表示已经读取到的字节数，<0表示错误，timeoutMs一般会自动根据数据流量设置
    //return value>0 readbytes indicates the number of bytes,When an error occurs value<0
    int ReadData(char* buf,unsigned int len,int timeoutMs=0);

    //返回值大于零返回表示已经发送到协议栈的字节数，<0表示错误 
    int SendData(const char* buf,unsigned int len,int timeoutMs=0);
