...
{
    return socket_send(ps,data,count,sent,tm);

}
int socket_read(p_socket ps, char *data, size_t count, size_t *got, p_timeout tm)
{
    return socket_recv(ps,data,count,got,tm);
}
...
bool CZSocket::Bind(const sockaddr_in *addr_in)
{
    ...
    static_assert(sizeof(sockaddr_in) == sizeof(SA),"sizeof(sockaddr_in) != sizeof(SA)");
...
int CZSocket::ReadData(char* buf,unsigned int len,int timeoutMs)
{
    if (timeoutMs<=0)
    {
        //timeoutMs=len/128+500;    //根据数据长度决定等待时间
        timeoutMs=len+30000;            //根据数据长度决定等待时间,按照网速最低10k/s，等待更长时间
    }
    t_timeout tm =timeoutMs;
    size_t getsize=0;

    //printf("m_socket.Receive() len:%d getsize:%d tm:%d begin\n",len,getsize,timeWait);
    //sTime.getBeginTime();
    int err = this->Receive2(buf,len,getsize,&tm);   //改阻塞才有时间相应消息
    //sTime.getEndPrint(1,true);
    //printf("m_socket.Receive() len:%d getsize:%d tm:%d end\n",len,getsize,timeWait);
    if (err==0)
    {
        return getsize;  //0
    }
...
int CZSocket::SendData(const char* buf,unsigned int len,int timeoutMs)
{
    size_t sent;
    if (timeoutMs<=0)
    {
    ...
    t_timeout tm =timeoutMs;
    int err = this->Send(buf, len,sent,&tm);
    if(sent<len)
    {
        printf("CZSocket::SendData() remainSize:%d\n",len-sent);
    }
    if(err<0) // if(err != 0)
    {
        printf("CZSocket::SendData() err:%d\n",err);
        //throw -1;
    }
    return err;
}
...
std::string CZSocket::getPeerIP()
{
    sockaddr_in peerAddr;
    int peerLen=sizeof(peerAddr);
    if(getpeername(m_socket,(struct sockaddr *)&peerAddr,&peerLen)==-1)
    {
...
unsigned short CZSocket::getPeerPort()
{
    ...
    if(getpeername(m_socket,(struct sockaddr *)&peerAddr,&peerLen)==-1)
    {
        //printf("getpeername error\n");
        //exit(0);
        return  0;    //无效端口号 failed 
    }
    return ntohs(peerAddr.sin_port);
}


void CZSocket::printIPInfo()
{
    char ip[32]={0};
    strcpy(ip,inet_ntoa(m_addrin.sin_addr));

    printf("Local IP:%s port:%d ip:%s\n",getIP().c_str(),getPort(),ip);
}
