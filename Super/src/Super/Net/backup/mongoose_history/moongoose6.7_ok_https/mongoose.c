...
extern enum cs_log_level m_cs_log_level;
...
#define DBG(x)                              \
  do {                                      \
    if (m_cs_log_level >= LL_VERBOSE_DEBUG) { \
      cs_log_print_prefix(__func__);        \
      cs_log_printf x;                      \
    }                                       \
  } while (0)
...
enum cs_log_level m_cs_log_level WEAK =
#if CS_ENABLE_DEBUG
    LL_VERBOSE_DEBUG;
#else
    LL_ERROR;
...
void MD5_Init_MonGoose(MD5_CTX_MonGoose *ctx) {
...
void MD5_Update_MonGoose(MD5_CTX_MonGoose *ctx, const unsigned char *buf, size_t len) {
  uint32_t t;
...
void MD5_Final_MonGoose(unsigned char digest[16], MD5_CTX_MonGoose *ctx) {
  unsigned count;
  unsigned char *p;
  uint32_t *a;
...
char *cs_md5(char buf[33], ...) {
  ...
  va_list ap;
  MD5_CTX_MonGoose ctx;

  MD5_Init_MonGoose(&ctx);

  va_start(ap, buf);
  while ((p = va_arg(ap, const unsigned char *) ) != NULL) {
    size_t len = va_arg(ap, size_t);
    MD5_Update_MonGoose(&ctx, p, len);
  }
  va_end(ap);

  MD5_Final_MonGoose(hash, &ctx);
...
struct mg_connection *mg_connect_http_base(
  ...
  //Fix to supported keepalive
  //Jaontolt
#define LC_KEEP_ALIVE 1
#ifdef LC_KEEP_ALIVE
  if(mgr->active_connections != NULL && !(mgr->active_connections->flags & MG_F_SEND_AND_CLOSE) && !(mgr->active_connections->flags & MG_F_CLOSE_IMMEDIATELY))
