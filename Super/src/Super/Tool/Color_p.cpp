...
bool get_hex_rgb(const char *name, Rgb_t *rgb)
{
    if(name[0] != '#')
        return false;
    name++;
...
bool get_hex_rgb(const char *str, int len, Rgb_t *rgb)
{
    if (len > 13)
        return false;
    char tmp[16];
...
static bool qt_get_named_rgb(const char *name_no_space, Rgb_t *rgb)
{
...
bool get_named_rgb(const char *name, Rgb_t* rgb)
{
    ...
    return qt_get_named_rgb(name_no_space, rgb);
}


bool get_named_rgb(const char *name, int len, Rgb_t *rgb)
{
    if(len > 255)
        return false;
    char name_no_space[256];
    ...
    for(int i = 0; i < len; i++) {
        if(name[i] != ('\t') && name[i] != (' '))
            name_no_space[pos++] = toupper(name[i]);
    }
    name_no_space[pos] = 0;
    return qt_get_named_rgb(name_no_space, rgb);
}
