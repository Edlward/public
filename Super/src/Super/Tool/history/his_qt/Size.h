...
public:
    ...
    void scale(int w, int h, Qt::AspectRatioMode mode) { scale(Size(w, h), mode); }
    void scale(const Size &s, Qt::AspectRatioMode mode) { *this = scaled(s, mode); }

    Size scaled(int w, int h, Qt::AspectRatioMode mode) const { return scaled(Size(w, h), mode); }
    Size scaled(const Size &s, Qt::AspectRatioMode mode) const
    {
...
public:
    ...
    void scale(qreal w, qreal h, Qt::AspectRatioMode mode) { scale(SizeF(w, h), mode); }
    void scale(const SizeF &s, Qt::AspectRatioMode mode) { *this = scaled(s, mode); }

    SizeF scaled(qreal w, qreal h, Qt::AspectRatioMode mode) const { return scaled(SizeF(w, h), mode); }
    SizeF scaled(const SizeF &s, Qt::AspectRatioMode mode) const
    {
        if (mode == Qt::IgnoreAspectRatio ||IsNull(wd) ||IsNull(ht)) {
            return s;
        } else {
            bool useHeight;
            qreal rw = s.ht * wd / ht;
