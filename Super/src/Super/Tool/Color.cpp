...
/*!
    ...
    There are 20 predefined QColors described by the Sp::GlobalColor enum,
    ...
    Additionally, the Sp::color0, Sp::color1 and Sp::transparent colors
    are used for special purposes.

    Sp::color0 (zero pixel value) and Sp::color1 (non-zero pixel value)
    are special colors for drawing in QBitmaps. Painting with Sp::color0
    sets the bitmap bits to 0 (transparent; i.e., background), and painting
    with Sp::color1 sets the bits to 1 (opaque; i.e., foreground).

    Sp::transparent is used to indicate a transparent pixel. When painting
 ...
 */
Color::Color(Sp::GlobalColor color)
{
#define QRGB(r, g, b) \
    Rgb_t(((0xffu << 24) | ((r & 0xff) << 16) | ((g & 0xff) << 8) | (b & 0xff)))
#define QRGBA(r, g, b, a) \
    Rgb_t(((a & 0xff) << 24) | ((r & 0xff) << 16) | ((g & 0xff) << 8) | (b & 0xff))

    static const Rgb_t global_colors[] = {
        QRGB(255, 255, 255), // Sp::color0
        QRGB(  0,   0,   0), // Sp::color1
        QRGB(  0,   0,   0), // black
...
//std::string QColor::name(NameFormat format) const
//{
//    switch (format) {
//    case HexRgb:
...
//std::string Color::name(NameFormat format) const
//{
//    char tmp[128];
//    switch (format) {
//    case HexRgb:
...
//       same as defined by the Sp::GlobalColor enums, e.g. "green" and Sp::green does not
...
std::vector<std::string> Color::colorNames()
{
...
*/
void Color::getHsvF(qreal *h, qreal *s, qreal *v, qreal *a) const
{
        if (!h || !s || !v)
        return;
...
*/
void Color::setHsvF(qreal h, qreal s, qreal v, qreal a)
{
    if (((h < qreal(0.0) || h > qreal(1.0)) && h != qreal(-1.0))
        ...
        printf("QColor::setHsvF: HSV parameters out of range\n");

        return;
    }

    cspec = Hsv;
    ct.ahsv.alpha      = Round(a * USHRT_MAX);
    ct.ahsv.hue        = h == qreal(-1.0) ? USHRT_MAX : Round(h * 36000);
    ct.ahsv.saturation = Round(s * USHRT_MAX);
    ct.ahsv.value      = Round(v * USHRT_MAX);
    ct.ahsv.pad        = 0;
}
...
void Color::setHsv(int h, int s, int v, int a)
{
    if (h < -1 || (uint)s > 255 || (uint)v > 255 || (uint)a > 255) 
    {
        ...
        printf("QColor::setHsv: HSV parameters out of range\n");
        invalidate();
        return;
    }
...
/*!
    ...
    \sa setHsl()
*/
void Color::getHslF(qreal *h, qreal *s, qreal *l, qreal *a) const
{
        if (!h || !s || !l)
        return;
...
/*!
    ...
    \sa getHslF(), setHsl()
*/
void Color::setHslF(qreal h, qreal s, qreal l, qreal a)
{
    if (((h < qreal(0.0) || h > qreal(1.0)) && h != qreal(-1.0))
        || (s < qreal(0.0) || s > qreal(1.0))
    ...
    cspec = Hsl;
    ct.ahsl.alpha      = Round(a * USHRT_MAX);
    ct.ahsl.hue        = h == qreal(-1.0) ? USHRT_MAX : Round(h * 36000);
    ct.ahsl.saturation = Round(s * USHRT_MAX);
    ct.ahsl.lightness  = Round(l * USHRT_MAX);
    ct.ahsl.pad        = 0;
}
...
/*!
    ...
    \sa rgb(), setRgb()
*/
void Color::getRgbF(qreal *r, qreal *g, qreal *b, qreal *a) const
{
    if (!r || !g || !b)
        return;
...
*/
void Color::setRgbF(qreal r, qreal g, qreal b, qreal a)
{
        ...
        || a < qreal(0.0) || a > qreal(1.0)) 
    {
        ...
        printf("QColor::setRgbF: RGB parameters out of range\n");
        invalidate();
        return;
    }

    cspec = Rgb;
    ct.argb.alpha = Round(a * USHRT_MAX);
    ct.argb.red   = Round(r * USHRT_MAX);
    ct.argb.green = Round(g * USHRT_MAX);
    ct.argb.blue  = Round(b * USHRT_MAX);
    ct.argb.pad   = 0;
}
...
void Color::setRgb(int r, int g, int b, int a)
{
    if ((uint)r > 255 || (uint)g > 255 || (uint)b > 255 || (uint)a > 255) 
    {
        ...
        printf("QColor::setRgb: RGB parameters out of range\n");
        invalidate();
        return;
    }
...
*/
qreal Color::hsvSaturationF() const 
{
    if (cspec != Invalid && cspec != Hsv)
...
*/
qreal Color::hslSaturationF() const 
{
    if (cspec != Invalid && cspec != Hsl)
    ...
    case Hsv:
        {
            ...
            if (i & 1) {
                ...
                switch (i) {
                case 1:
                    color.ct.argb.red   = Round(q * USHRT_MAX);
                    color.ct.argb.green = Round(v * USHRT_MAX);
                    color.ct.argb.blue  = Round(p * USHRT_MAX);
                    break;
                case 3:
                    color.ct.argb.red   = Round(p * USHRT_MAX);
                    color.ct.argb.green = Round(q * USHRT_MAX);
                    color.ct.argb.blue  = Round(v * USHRT_MAX);
                    break;
                case 5:
                    color.ct.argb.red   = Round(v * USHRT_MAX);
                    color.ct.argb.green = Round(p * USHRT_MAX);
                    color.ct.argb.blue  = Round(q * USHRT_MAX);
                    break;
                }
            } else {
                ...
                case 0:
                    color.ct.argb.red   = Round(v * USHRT_MAX);
                    color.ct.argb.green = Round(t * USHRT_MAX);
                    color.ct.argb.blue  = Round(p * USHRT_MAX);
                    break;
                case 2:
                    color.ct.argb.red   = Round(p * USHRT_MAX);
                    color.ct.argb.green = Round(v * USHRT_MAX);
                    color.ct.argb.blue  = Round(t * USHRT_MAX);
                    break;
                case 4:
                    color.ct.argb.red   = Round(t * USHRT_MAX);
                    color.ct.argb.green = Round(p * USHRT_MAX);
                    color.ct.argb.blue  = Round(v * USHRT_MAX);
                    break;
                }
            ...
            if (ct.ahsl.saturation == 0 || ct.ahsl.hue == USHRT_MAX) {
                ...
                for (int i = 0; i != 3; ++i) {
                    ...
                    if (sixtemp3 < qreal(1.0))
                        color.ct.array[i+1] = Round((temp1 + (temp2 - temp1) * sixtemp3) * USHRT_MAX);
                    else if ((temp3[i] * qreal(2.0)) < qreal(1.0))
                        color.ct.array[i+1] = Round(temp2 * USHRT_MAX);
                    else if ((temp3[i] * qreal(3.0)) < qreal(2.0))
                        color.ct.array[i+1] = Round((temp1 + (temp2 -temp1) * (qreal(2.0) /qreal(3.0) - temp3[i]) * qreal(6.0)) * USHRT_MAX);
                    else
                        color.ct.array[i+1] = Round(temp1 * USHRT_MAX);
                }
    ...
    case Cmyk:
        {
            ...
            color.ct.argb.red   =Round((qreal(1.0) - (c * (qreal(1.0) - k) + k)) * USHRT_MAX);
            color.ct.argb.green =Round((qreal(1.0) - (m * (qreal(1.0) - k) + k)) * USHRT_MAX);
            color.ct.argb.blue  =Round((qreal(1.0) - (y * (qreal(1.0) - k) + k)) * USHRT_MAX);
            break;
        }
    default:
        break;
...
Color Color::toHsv() const 
{
    ...
    color.ct.ahsv.value =Round(max * USHRT_MAX);
    if (FuzzyIsNull(delta)) {
        ...
        // chromatic case
        qreal hue = 0;
        color.ct.ahsv.saturation =Round((delta / max) * USHRT_MAX);
        if (FuzzyCompare(r, max)) {
            ...
            S_ASSERT_X(false, "QColor::toHsv", "internal error");
        }
        hue *= qreal(60.0);
        if (hue < qreal(0.0))
            hue += qreal(360.0);
        color.ct.ahsv.hue = Round(hue * 100);
    }
...
Color Color::toHsl() const 
{
    ...
    color.ct.ahsl.lightness = Round(lightness * USHRT_MAX);
    if (FuzzyIsNull(delta)) {
        // achromatic case, hue is undefined
        ...
        if (lightness < qreal(0.5))
            color.ct.ahsl.saturation = Round((delta / delta2) * USHRT_MAX);
        else
            color.ct.ahsl.saturation = Round((delta / (qreal(2.0) - delta2)) * USHRT_MAX);
        if (FuzzyCompare(r, max)) {
            hue = ((g - b) /delta);
        } else if (FuzzyCompare(g, max)) {
            hue = (qreal(2.0) + (b - r) / delta);
        ...
        if (hue < qreal(0.0))
            hue += qreal(360.0);
        color.ct.ahsl.hue = Round(hue * 100);
    }
...
Color Color::toCmyk() const 
{
    ...
    color.ct.acmyk.cyan    = Round(c * USHRT_MAX);
    color.ct.acmyk.magenta = Round(m * USHRT_MAX);
    color.ct.acmyk.yellow  = Round(y * USHRT_MAX);
    color.ct.acmyk.black   = Round(k * USHRT_MAX);
...
Color Color::fromRgba(Rgb_t rgba) 
{
...
Color Color::fromRgb(int r, int g, int b, int a)
{
        ...
        || a < 0 || a > 255) 
    {
        ...
        printf("QColor::fromRgb: RGB parameters out of range\n");
        return Color();
    }
...
*/
Color Color::fromRgbF(qreal r, qreal g, qreal b, qreal a)
{
    if (r < qreal(0.0) || r > qreal(1.0)
        || g < qreal(0.0) || g > qreal(1.0)
        || b < qreal(0.0) || b > qreal(1.0)
        || a < qreal(0.0) || a > qreal(1.0)) 
    {
        ...
        printf("QColor::fromRgbF: RGB parameters out of range\n");
        return Color();
    }

    Color color;
    color.cspec = Rgb;
    color.ct.argb.alpha = Round(a * USHRT_MAX);
    color.ct.argb.red   = Round(r * USHRT_MAX);
    color.ct.argb.green = Round(g * USHRT_MAX);
    color.ct.argb.blue  = Round(b * USHRT_MAX);
    color.ct.argb.pad   = 0;
    return color;
}
...
Color Color::fromHsv(int h, int s, int v, int a)
{
        ...
        || a < 0 || a > 255) 
    {
        ...
        printf("QColor::fromHsv: HSV parameters out of range\n");
        return Color();
    }
...
*/
Color Color::fromHsvF(qreal h, qreal s, qreal v, qreal a)
{
    if (((h < qreal(0.0) || h > qreal(1.0)) && h != qreal(-1.0))
        || (s < qreal(0.0) || s > qreal(1.0))
        || (v < qreal(0.0) || v > qreal(1.0))
        ...
        printf("QColor::fromHsvF: HSV parameters out of range\n");
        return Color();
    }

    Color color;
    color.cspec = Hsv;
    color.ct.ahsv.alpha      = Round(a * USHRT_MAX);
    color.ct.ahsv.hue        = h == qreal(-1.0) ? USHRT_MAX : Round(h * 36000);
    color.ct.ahsv.saturation = Round(s * USHRT_MAX);
    color.ct.ahsv.value      = Round(v * USHRT_MAX);
    color.ct.ahsv.pad        = 0;
    return color;
}
...
Color Color::fromHsl(int h, int s, int l, int a)
{
        ...
        || a < 0 || a > 255) 
    {
        ...
        printf("QColor::fromHsl: HSL parameters out of range\n");
        return Color();
    }
...
*/
Color Color::fromHslF(qreal h, qreal s, qreal l, qreal a)
{
    if (((h < qreal(0.0) || h > qreal(1.0)) && h != qreal(-1.0))
        || (s < qreal(0.0) || s > qreal(1.0))
        || (l < qreal(0.0) || l > qreal(1.0))
        || (a < qreal(0.0) || a > qreal(1.0))) 
    {
        ...
        printf("QColor::fromHslF: HSL parameters out of range\n");
        return Color();
    }

    Color color;
    color.cspec = Hsl;
    color.ct.ahsl.alpha      = Round(a * USHRT_MAX);
    color.ct.ahsl.hue        = (h == qreal(-1.0)) ? USHRT_MAX : Round(h * 36000);
    if (color.ct.ahsl.hue == 36000)
        color.ct.ahsl.hue = 0;
    color.ct.ahsl.saturation = Round(s * USHRT_MAX);
    color.ct.ahsl.lightness  = Round(l * USHRT_MAX);
    color.ct.ahsl.pad        = 0;
    return color;
}
...
*/
void Color::getCmyk(int *c, int *m, int *y, int *k, int *a)
{
    if (!c || !m || !y || !k)
        return;
...
*/
void Color::getCmykF(qreal *c, qreal *m, qreal *y, qreal *k, qreal *a)
{
    if (!c || !m || !y || !k)
        return;
...
void Color::setCmyk(int c, int m, int y, int k, int a)
{
        ...
        || a < 0 || a > 255) 
    {
        ...
        printf("QColor::setCmyk: CMYK parameters out of range\n");
        return;
    }
...
*/
void Color::setCmykF(qreal c, qreal m, qreal y, qreal k, qreal a)
{
        ...
        || a < qreal(0.0) || a > qreal(1.0)) 
    {
        ...
        printf("QColor::setCmykF: CMYK parameters out of range\n");
        return;
    }

    cspec = Cmyk;
    ct.acmyk.alpha   = Round(a * USHRT_MAX);
    ct.acmyk.cyan    = Round(c * USHRT_MAX);
    ct.acmyk.magenta = Round(m * USHRT_MAX);
    ct.acmyk.yellow  = Round(y * USHRT_MAX);
    ct.acmyk.black   = Round(k * USHRT_MAX);
}
...
Color Color::fromCmyk(int c, int m, int y, int k, int a)
{
        ...
        || a < 0 || a > 255) 
    {
        ...
        printf("QColor::fromCmyk: CMYK parameters out of range\n");
        return Color();
    }
...
*/
Color Color::fromCmykF(qreal c, qreal m, qreal y, qreal k, qreal a)
{
    if (c < qreal(0.0) || c > qreal(1.0)
        || m < qreal(0.0) || m > qreal(1.0)
        || y < qreal(0.0) || y > qreal(1.0)
        ...
        || a < qreal(0.0) || a > qreal(1.0))
    {
        ...
        printf("QColor::fromCmykF: CMYK parameters out of range\n");
        return Color();
    }

    Color color;
    color.cspec = Cmyk;
    color.ct.acmyk.alpha   = Round(a * USHRT_MAX);
    color.ct.acmyk.cyan    = Round(c * USHRT_MAX);
    color.ct.acmyk.magenta = Round(m * USHRT_MAX);
    color.ct.acmyk.yellow  = Round(y * USHRT_MAX);
    color.ct.acmyk.black   = Round(k * USHRT_MAX);
 ...
 */
Color &Color::operator=(Sp::GlobalColor color) 
{
...
bool Color::operator==(const Color &color) const 
{
    if (cspec == Hsl && cspec == color.cspec) {
        return (ct.argb.alpha == color.ct.argb.alpha
                ...
                && (Abs(ct.ahsl.saturation - color.ct.ahsl.saturation) < 50
                    || ct.ahsl.lightness == 0
                    || color.ct.ahsl.lightness == 0
                    || ct.ahsl.lightness == USHRT_MAX
                    || color.ct.ahsl.lightness == USHRT_MAX)
                && (Abs(ct.ahsl.lightness - color.ct.ahsl.lightness)) < 50);
    } else {
