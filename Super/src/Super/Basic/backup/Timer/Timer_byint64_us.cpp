...
static const int64_t NumusPerms=1000;    //每ms的us数量
static const int64_t dis2ms=2*NumusPerms;      //差距2ms
static const int64_t dismin=0.1*NumusPerms;     //最小差距0.1ms
static const int64_t disminus=0.01*NumusPerms;     //最小差距0.01ms
...
//time in ns 为了时间差对比简单 返回类型转为int64_t
static inline int64_t currTimeus()
{
    return SuperTime::getStartupToNowInus();
}

//time in ns 
static inline int64_t currTimens()
{
...
{
    alreadyCount=0;
    begin_time=currTimeus();            //定时器开始时绝对时间
...
{
    alreadyCount++;
    int64_t curr=currTimeus();
    //本次在回调函数里面超时了，超时次数统计或时间累加有需要待续。
    //uint64_t  currUsedtime=curr-(calltime-peroid);    //当前时间减去回调前时间
    int64_t  currUsedtime=curr-in_time;    //当前时间减去回调前时间
...
int Timer::doAddTimer(double PeriodInus,TimerCallFun pCbFun,void* pUser,void* pData,uint32_t times)
{
    //小于0的时间周期没有意义
    if (PeriodInus<=0)
    {
    ...
    PRINT_FUN_NAME_GOON
        printf("Period:%.16f ms id:%u\n",PeriodInus,Tinfo.timerID);
...
void Timer::TimerThread::run()
{
    ...
    while (getContinueRun())
    {
        ...
        {
            TimerInfo& Tinfo=VTimer[n];
            int64_t timeDiff=Tinfo.calltime-currTimeus();
            if (timeDiff>dismin)  //与理论时间差距0.1ms ,则调用定时回调函数
            {
                continue;
            }
...
int MediaTimer::doSetTimer(double PeriodInus,TimerCallFun pCbFun,void* pUser,void* pData,uint32_t times)
{
    //小于0的时间周期没有意义
    if (PeriodInus<=0)
    {
...
void MediaTimer::TimerThread::run()
{
    ...
    {
        int64_t timeDiff=Tinfo.calltime-currTimeus();
        if (timeDiff>dis2ms)               //差距比较大用系统延时
        {
            //Thread::sleep_for(1);        //or sleep(0)
            //Sp.sleep_for(1);                //faster
            Sp.sleep_for((timeDiff/NumusPerms)/2.0); //为了减轻负荷用sleep延时剩余时间的一半。逐步逼近
            continue;
        }
        if (timeDiff>disminus)   //不延时,轮询 //与理论时间差距小于0.1ms ,则调用定时回调函数
        {
            continue;
        }
...
int TimerDynamic::doAddTimer(double PeriodInus,TimerCallFun pCbFun,void* pUser,void* pData,uint32_t times)
{
    //小于0的时间周期没有意义
    if (PeriodInus<=0)
    {
...
void TimerDynamic::TimerThread::run()
{
    ...
    while (getContinueRun())
    {
        ...
        for (size_t n=0; n<ListTimer.size();n++)
        {
            ...
            int64_t timeDiff=Tinfo.calltime-currTimeus();
            if (timeDiff>dismin)  //与理论时间差距1ms ,则调用定时回调函数
            {
                continue;
            }
