...
//周期用double记录精度更高,时间点用uint64_t纳秒，速度快，
//再改,为了长时间精度,统一单位为us 

struct  TimerInfo
{
    int timerID;                       //每个定时器返回唯一ID
    double peroid;                  //定时器回调周期,单位us 微妙,周期用double记录精度更高
    int64_t begin_time;        //本定时器开始时的绝对时间,首次使用刷新
    int64_t calltime;             //回调到期时间
    int64_t in_time;              //调用前记录，用在计算函数调用函数，仅调试用
    int32_t timerCount;        //本定时器会运行几次  默认0xFFFFFFFF;表示永远运行,为0将不会被运行
    int32_t alreadyCount;     //已经运行了多少次
...
public:
    ...
    int SetTimer(double PeriodInms,TimerCallFun pCbFun=0,void* pUser=0,void* pData=0,uint32_t times=UINT32_MAX)
    {
        ...
        return doAddTimer(1000*PeriodInms,pCbFun,pUser,pData,times);
    }
    ...
    {
        return doAddTimer((double)PeriodInms*1000,pCbFun,pUser,pData,times);
    }
    int AddTimerHz(unsigned int FreqHz,TimerCallFun pCbFun=0,void* pUser=0,void* pData=0,uint32_t times=UINT32_MAX)         //直接指定周期
    {
        return doAddTimer(1000000.0/FreqHz,pCbFun,pUser,pData,times);
    }
    //外部调用
...
private:
    //为了精度与效率,统一调用内部用us精度接口实现
    int doAddTimer(double PeriodInus,TimerCallFun pCbFun=0,void* pUser=0,void* pData=0,uint32_t times=UINT32_MAX);
...
private:
    //为了精度与效率,统一调用内部用us精度接口实现
    int doSetTimer(double PeriodInus,TimerCallFun pCbFun=0,void* pUser=0,void* pData=0,uint32_t times=UINT32_MAX);
