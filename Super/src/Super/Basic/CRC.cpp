...
//from CSDN CRC16常见几个标准的算法及C语言实现   to tested
...
static uint16_t invert_word( uint16_t  U8B ){
...
static uint32_t  invert_dword( uint32_t  U8B ){
...
//void InvertUint8(unsigned char * p1,unsigned char * p2)
//{
//     *p2=invert_byte(*p1);
//};
//void InvertUint16(unsigned short* p1,unsigned short*  p2)
//{
//     *p2=invert_word(*p1);
//};
...
unsigned short CRC16_XModemByTable(const void *pData, int ilength)
{
    ...
    unsigned char *puchData=(unsigned char*)pData;
...
//uint16_t crc16(const unsigned char *buf,unsigned short length)
//{
...
/*
//static const unsigned int crc_tab32[] =
static unsigned int crc_tab32[] =
{
...
int init_crc32_tab()
{
    ...
    printf("init_crc32_tab() finish\n");

    return 0;   //添加返回值  可以初始化
}


static int initVal=init_crc32_tab();   //main前初始化

/*计算buffer的crc校验码*/
uint32_t Get_CRC32_fromBuffer(uint32_t crcinit, uint8_t * bs, uint32_t bssize)
{
    if(bssize==0) return 0;  //add by lizulin
...
//计算大文件的CRC校验码
uint32_t GetCRC32_fromFile(FILE *fd)
{
    const uint32_t bufferSize =16*1024;
    uint8_t crcbuf[bufferSize];
    size_t rdlen;
    uint32_t crc = 0;//CRC初始值为0
    while((rdlen=fread(crcbuf, sizeof(uint8_t),bufferSize, fd)) > 0)
    {
...
Output: 11E60398 hex
		都默默的进行过了模65521操作了
*/
unsigned int adler32(const unsigned char *data, unsigned int len)   //例子(一个不高效，但是很直接的实现):
{
