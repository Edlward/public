...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
//显示共享，写不会触发detach() 需要手动调用--案例待续


//
//QExplicitlySharedDataPointer <T>使编写自己的显式共享类变得容易。 QExplicitlySharedDataPointer实现了线程安全的引用计数，确保将QExplicitlySharedDataPointers添加到可重入的类中不会使它们成为不可重入的。
//
//除了一个很大的区别，QExplicitlySharedDataPointer就像QSharedDataPointer一样。最大的区别在于，在允许修改共享数据对象之前，QExplicitlySharedDataPointer的成员函数不会执行QSharedDataPointer的非const成员所执行的写入时自动复制操作（detach（））。有一个detach（）函数，但是如果您确实要detach（），则必须自己调用它。这意味着QExplicitlySharedDataPointers的行为类似于常规C ++指针，除了通过进行引用计数并在引用计数为0之前不删除共享数据对象之外，它们避免了悬空指针问题。
//
//通过示例比较QExplicitlySharedDataPointer与QSharedDataPointer是有益的。考虑QSharedDataPointer中的Employee示例，该示例已修改为使用显式共享，如讨论隐式与显式共享中所述。
//
//请注意，如果使用此类但发现经常调用detach（），则可能应该使用QSharedDataPointer。
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
//Qt 隐式共享学习 
...
/*
如何编写具有隐式共享特性的类？
以前我用纯 C++ 写过，但是当时考虑的很多，而且代码不好理解。
Qt 有 QSharedData 和 QSharedDataPointer 类可以方便的实现隐式共享。
这两个该类本身就实现了线程安全的引用计数，所以很多细节的东西不需要再去考虑了。
...
Employee 类对象的数据都从 d 指针访问。其中的写操作会自动调用 detach() 函数，
这样共享数据对象的引用计数大于1会创建共享数据对象的副本。
因为继承 QSharedData，所以内部会有个引用计数器。一般来说，
实现隐式共享机制必须有默认构造函数、拷贝构造函数、析构函数三个。
*/


#include <string>

    class EmployeeData : public SharedData
    {
    public:
        EmployeeData() : id(-1) {}
        EmployeeData(const EmployeeData &other):SharedData(other), id(other.id), name(other.name) {}
        ~EmployeeData();
        int id;
        std::string name;
    };
    ...
    public:
        ...
        std::string name() {return d->name;}
        void setName(const std::string &name) {d->name = name;}
    private:
        SharedDataPointer<EmployeeData> d;
    };
