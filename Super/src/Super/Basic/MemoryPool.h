...
class MemPool
{
    ...
    {
        size_t memSize; 
        MemBlock* pBlock;   //记住所在Block指针,便于快速释放,而不是根据大小寻找
    ...
    public:
        MemChunk(void* pBufRaw,size_t nMemSize)
        {
            ...
            this->pUser=(char*)pData+sizeof(MemHeader);    
    ...
    public:
        //默认不创建，需要时会自动扩容
        MemBlock(MemPool* pMemPool,size_t nChunkSize,size_t nChunkNumber=0)  
        ...
        void* Malloc()
        {
            ...
            //从空闲列表取一个元素
            MemChunk* pChunk=ListFree.back();        //取最后一个元素，
        ...
        void Free(MemChunk* pChunk)   //fast
        {
            //释放的还原到空闲列表
        ...
        void FreeOld(void* p)
        {
            ...
            MemChunk* pParentChunk=pHead->pChunk; //取得所在Chunk地址
            //释放的还原到空闲列表
    ...
    public:
        ...
        size_t ChunkMemUsed()const {return m_ChunkSize+sizeof(MemHeader);}  
        ...
        void printStatus()
        {
            printf("MemBlock ChunkInfo:Size:%llu Sum:%llu Used:%llu Unused:%llu \n"
    ...
    private:
        ...
        std::vector<MemChunk*> ListFree;   //空闲分块列表
...
public:
    ...
    void InitBlock()
    {
        ...
        //每一个索引管理<=2^n-1的内存；0，1，3，7，15，31.
         ...
         for (unsigned int nBlock=0;nBlock<64*16;nBlock++)
         {
             ...
             MemBlock* p=new MemBlock(this,blockMgrSize,0);   
             ...
             if (tBlock!=nBlock)
             {
                 //PreDprintf("May Error at:BlockNumToSize<->SizeToBlockNum\n");   //可能大小与分块号的分配策略错误
             }
         }
    }
    ...
    void* doMallocFromSystem(size_t nMemSize)
        {
            ...
            size_t nActuAlallocSize=sizeof(MemHeader)+nMemSize;
    ...
    void* doMallocAligned(size_t memSize,const int alignBytes=32)  //任意字节对齐版本
    {
        typedef int OffsetType;   //用int 型保存距离偏差
        char *ptrRaw=(char*)doMalloc(memSize+alignBytes+sizeof(OffsetType));
...
protected:
private:
    std::vector<MemBlock*> ListMemBlock;    //各个分块容器
...
public:
    ...
    //uint32_t index=(h1pos<<(N-1))+subIndex;   
    //return index;
    //}
    template <const size_t N>  //N=1,2,3,4,5
...
public:
    static void TestBlockNumAndSize1()
    {
        PRINT_FUN_NAME
        const unsigned int TestNum=65536*10;
        for (unsigned int n=0;n<TestNum;n++)
        {
            uint64_t nSize=n;
            if (n>10*1024)
            {
            ...
            unsigned int blockNum=SizeToBlockNumStrategy<1>(nSize);
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<1>(blockNum);
            //printf("Strategy S1:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            if (nSize>BlockMgrMaxSize)
            {
                printf("Error Strategy S1:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            }
        }
        for (unsigned int n=0;n<TestNum;n++)
        {
            ...
            unsigned int blockNum=SizeToBlockNumStrategy<2>(nSize);
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<2>(blockNum);
            //printf("Strategy S2:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            if (nSize>BlockMgrMaxSize)
            {
                printf("Error Strategy S2:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            }
        }
        for (unsigned int n=0;n<TestNum;n++)
        {
            ...
            unsigned int blockNum=SizeToBlockNumStrategy<3>(nSize);
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<3>(blockNum);
            //printf("Strategy S3:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            if (nSize>BlockMgrMaxSize)
            {
                printf("Error Strategy S3:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            }
        }
        for (unsigned int n=0;n<TestNum;n++)
        {
            ...
            unsigned int blockNum=SizeToBlockNumStrategy<4>(nSize);
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<4>(blockNum);
            //printf("Strategy S4:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            if (nSize>BlockMgrMaxSize)
            {
                printf("Error Strategy S4:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            }
        }
        for (unsigned int n=0;n<TestNum;n++)
        {
            ...
            unsigned int blockNum=SizeToBlockNumStrategy<5>(nSize);
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<5>(blockNum);
            //printf("Strategy S5:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            if (nSize>BlockMgrMaxSize)
            {
                printf("Error Strategy S5:Size:%llu Block:%03u BlockMgrSize:%llu\n",nSize,blockNum,BlockMgrMaxSize);
            }
        }
    }
    static void TestBlockNumAndSize2()
    {
        PRINT_FUN_NAME
        for (unsigned int blockNum=0;blockNum<64*64;blockNum++)
        {
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<1>(blockNum);
            uint32_t nblock=SizeToBlockNumStrategy<1>(BlockMgrMaxSize);
            //printf("BlockToSize S1:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
            if (nblock>blockNum)
            {
                 printf("Error BlockToSize S1:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
                //Thread::sleep_for(3000);
            }
            if (BlockMgrMaxSize>=UINT64_MAX)
            {
                break;
            }
        }
        ...
        {
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<2>(blockNum);
            uint32_t nblock=SizeToBlockNumStrategy<2>(BlockMgrMaxSize);
            //printf("BlockToSize S2:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
            if (nblock>blockNum)
            {
                printf("Error BlockToSize S2:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
                //Thread::sleep_for(3000);
            }
            if (BlockMgrMaxSize>=UINT64_MAX)
            {
        ...
        {
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<3>(blockNum);
            uint32_t nblock=SizeToBlockNumStrategy<3>(BlockMgrMaxSize);
            //printf("BlockToSize S3:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
            if (nblock>blockNum)
            {
                printf("Error BlockToSize S3:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
                //Thread::sleep_for(3000);
            }
            if (BlockMgrMaxSize>=UINT64_MAX)
            {
        ...
        {
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<4>(blockNum);
            uint32_t nblock=SizeToBlockNumStrategy<4>(BlockMgrMaxSize);
            //printf("BlockToSize S4:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
            if (nblock>blockNum)
            {
                printf("Error BlockToSize S4:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
                //Thread::sleep_for(3000);
            }
            if (BlockMgrMaxSize>=UINT64_MAX)
            {
        ...
        {
            uint64_t BlockMgrMaxSize=BlockNumToSizeStrategy<5>(blockNum);
            uint32_t nblock=SizeToBlockNumStrategy<5>(BlockMgrMaxSize);
            //printf("BlockToSize S5:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
            if (nblock>blockNum)
            {
                printf("Error BlockToSize S5:Block:%03u MgrSize:%llu rBlock%03u\n",blockNum,BlockMgrMaxSize,nblock);
                //Thread::sleep_for(3000);
            }
            if (BlockMgrMaxSize>=UINT64_MAX)
            {
    ...
    static void TestBlockDevideStrategy()
    {
        ...
        TestBlockNumAndSize1();
        TestBlockNumAndSize2();
...
inline bool MemPool<DevideType>::MemBlock::AppendChunkSpace(size_t nChunkNumber)
{
    for (size_t n=0;n<nChunkNumber;n++)
    {
        //MemChunk* pChunk=new MemChunk(m_ChunkSize);

        size_t newChunkmemSize=sizeof(MemHeader)+m_ChunkSize;
        ...
        MemChunk* pChunk=new MemChunk(pBufRaw,m_ChunkSize);
